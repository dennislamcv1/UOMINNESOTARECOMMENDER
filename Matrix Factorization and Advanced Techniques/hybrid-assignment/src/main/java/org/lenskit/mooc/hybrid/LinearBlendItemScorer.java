...
public class LinearBlendItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items) {
        ...
        ResultMap leftResults = leftScorer.scoreWithDetails(user, items);
        ResultMap rightResults = rightScorer.scoreWithDetails(user, items);
        double userBias = biasModel.getIntercept() + biasModel.getUserBias(user);

        for (long item : items) {
            double baseline = userBias + biasModel.getItemBias(item);

            Result left = leftResults.get(item);
            double leftOffset = left == null ? 0 : left.getScore() - baseline;

            Result right = rightResults.get(item);
            double rightOffset = right == null ? 0 : right.getScore() - baseline;

            double score = baseline
                    + (1 - blendWeight) * leftOffset
                    + blendWeight * rightOffset;
